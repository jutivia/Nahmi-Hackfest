...
import {ASSET_TOKEN_CONTRACT, ASSET_TOKEN_ADDRESS, BOND_DEPO_ADDRESS, BOND_DEPO_CONTRACT, STAKING_CONTRACT, STAKING_ADDRESS, NIIT_ADDRESS, NIIT_CONTRACT } from '../utils/constants/constants'
...
function Web3ContextProvider({ children }) {
    ...
    const getAssetTokenBalance = async (address) => {
     ...
     const getNIITBalance = async (address) => {
        if (!connected) {
            try {
                const contract = getContractWithSigner(NIIT_ADDRESS, NIIT_CONTRACT);
                console.log(contract)
                const balance = await contract.balanceOf(address);
    ...
    const checkExistingBond =async (addr) =>{
     ...
     const stakeTokens =async (amount) =>{
         try {
                const contract = getContractWithSigner(STAKING_ADDRESS, STAKING_CONTRACT);
                const niitContract = getContractWithSigner(NIIT_ADDRESS, NIIT_CONTRACT);
                await niitContract.approve(STAKING_ADDRESS, utils.parseEther(amount));
    ...
    const withdrawStakedTokens =async (amount) =>{
         try {
                const contract = getContractWithSigner(STAKING_ADDRESS, STAKING_CONTRACT);
                const res = await contract.withdrawStake(ethers.utils.parseEther(amount));
                await res.wait();
                toast.success('Transaction successfull')
                const coinBalance = await getCoinBalance(account);
                const assetTokenBalance = await getAssetTokenBalance(account);
